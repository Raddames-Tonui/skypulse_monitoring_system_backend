    ...
    private void upsertSslLog(Connection c, long serviceId, String host, Map<String,Object> certInfo,
        ...
        String update = """
            UPDATE ssl_logs SET
              ...
              san_list = ?, chain_valid = ?, expiry_date = ?, days_remaining = ?, last_checked = now(), date_modified = now()
        ...
        try (PreparedStatement ps = c.prepareStatement(update)) {
            ...
            ps.setDate(8, Date.valueOf(expiry.toLocalDate()));
            ps.setInt(9, daysRemaining);
            ps.setLong(10, serviceId);
            ps.setString(11, host);
            if (ps.executeUpdate() > 0) return;
        }

        String insert = """
            INSERT INTO ssl_logs(monitored_service_id, domain, issuer, serial_number, signature_algorithm,
                                 public_key_algo, public_key_length, san_list, chain_valid, expiry_date, days_remaining,
        ...
        try (PreparedStatement ps = c.prepareStatement(insert)) {
            ...
            ps.setDate(10, Date.valueOf(expiry.toLocalDate()));
            ps.setInt(11, daysRemaining);
            ps.executeUpdate();
        }
    }
